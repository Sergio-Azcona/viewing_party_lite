...
RSpec.describe TopMovie do
  ...
  describe 'returns title, vote_average, and id values for movies in the array' do
    it 'returns the movies in vote_average descending order' do
      data = 
      {
      "page": 1,
      "results": [
      ...
      movie_1 = TopMovie.new(data[:results].first)
      movie_2 = TopMovie.new(data[:results].second)
      movie_3 = TopMovie.new(data[:results].third)
      movie_20 = TopMovie.new(data[:results].last)
      vote_averge_range = (data[:results].pluck(:vote_average))
              
      expect(movie_1.title).to eq('The Godfather')
      expect(movie_1.vote_average).to eq(8.7)
      expect(movie_1.id).to eq(238)

      #unique movies can have the same vote_average, but not id (and likely not titles)
      expect(movie_1.vote_average).to eq(movie_2.vote_average)
      expect(movie_1.title).to_not eq(movie_2.title)
      expect(movie_1.id).to_not eq(movie_2.id)

      #response is ordered by vote_average (desc)
      expect(vote_averge_range.uniq).to eq([8.7, 8.6, 8.5])
      expect(movie_1.vote_average).to_not eq(movie_3.vote_average)
      expect(movie_3.vote_average).to eq(8.6)
      expect(movie_20.vote_average).to eq(8.5)
    end
  end
